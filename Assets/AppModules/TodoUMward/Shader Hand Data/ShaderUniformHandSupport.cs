      ...
      if (leftHand != null) {
        ...
        // Palm Tris
        {
          ...
          // Left Hand Palm Tris
          {
            ...
            var distalAxisShift = -hand.DistalAxis() * 0.015f;

            // Thumb Tri
            palmTris[i++] = thumb.bones[1].NextJoint.ToVector3().WithW(1);
            palmTris[i++] = thumb.bones[1].PrevJoint.ToVector3().WithW(1);
            palmTris[i++] = Vector3.Lerp(index.bones[0].PrevJoint.ToVector3() + distalAxisShift, index.bones[0].NextJoint.ToVector3(), 0.8f).WithW(1);

            // Index Tri
            palmTris[i++] = index.bones[0].NextJoint.ToVector3().WithW(1);
            palmTris[i++] = (index.bones[0].PrevJoint.ToVector3() + distalAxisShift).WithW(1);
            palmTris[i++] = (pinky.bones[0].PrevJoint.ToVector3() + distalAxisShift).WithW(1);
            ...
            palmTris[i++] = ring.bones[0].NextJoint.ToVector3().WithW(1);
            palmTris[i++] = (pinky.bones[0].PrevJoint.ToVector3() + distalAxisShift).WithW(1f);
          }
        }
