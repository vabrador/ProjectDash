  ...
  public void Duplicate() {
    ...
    for (int i = 0; i < numHeightCopies; i++) {
      for (int j = 0; j < numWidthCopies; j++) {
        for (int r = 0; r < numRotateCopies; r++) {
          var position = toDuplicate.transform.position +
            j * horizontalSpacing * duplicationParent.right *
              this.transform.lossyScale.x +
            i * verticalSpacing * -duplicationParent.up *
              this.transform.lossyScale.y;
          
          Quaternion rotation = Quaternion.AngleAxis(rotationSpacing * r,
          ...
          duplicate.transform.rotation = toDuplicate.transform.rotation*rotation;
